...
function run_mobile_tasks_only() {
  ...
  const mobileBtn = document.getElementById('mobile_tasks_btn');
  const stopBtn = document.getElementById('stop_btn');
  if (startBtn) startBtn.disabled = true;
  if (mobileBtn) mobileBtn.disabled = true;
  if (stopBtn) stopBtn.disabled = false;
  update_status_indicator('executing');
  show_toast('Running mobile check-in and Read to Earn…', 'info', { duration: 5000 });
  pywebview.api.main(0, 0, false, true, true);
}
...
function enable_start_button() {
  ...
  const mobileBtn = document.getElementById('mobile_tasks_btn');
  if (mobileBtn) {
    mobileBtn.disabled = !(current && current.first_setup_done) || driverWarmingUp || balanceFetching;
  }
...
function set_stats_loading(on) {
  ...
  if (balanceFetching) {
    ...
    const mobileBtn = document.getElementById('mobile_tasks_btn');
    if (mobileBtn) mobileBtn.disabled = !(current && current.first_setup_done) || driverWarmingUp;
  }
}
...
function format_schedule_summary(item, sched, enabled) {
  ...
  const mobileTasks = sched.mobile_tasks != null
    ? (typeof sched.mobile_tasks === 'object' ? sched.mobile_tasks : { enabled: Boolean(sched.mobile_tasks) })
    : (item.mobile_tasks || {});
  const mobileSuffix = mobileTasks.enabled ? ' · Mobile tasks' : '';
  if (!enabled) return prefix + 'Schedule off' + mobileSuffix;
  ...
  if (sched.advancedScheduling) {
    ...
    return `${prefix}${time} · PC ${pc} / Mobile ${mobile} · ${dur}h @ ${qph}/h${mobileSuffix}`;
  }
  return `${prefix}${time} · PC ${pc} / Mobile ${mobile}${mobileSuffix}`;
}
...
function build_schedule_card(item) {
  ...
  // Mobile Rewards API tasks. These are independent of browser search counts
  // and may therefore be enabled even when PC/Mobile queries are both zero.
  const mobileCfg = item.mobile_tasks || {};
  const mobileGroup = document.createElement('div');
  mobileGroup.className = 'mobile-task-settings';
  const mobileTitle = document.createElement('div');
  mobileTitle.className = 'settings-group-title';
  mobileTitle.textContent = 'Mobile tasks';
  mobileGroup.appendChild(mobileTitle);

  const mobileEnableRow = document.createElement('label');
  mobileEnableRow.className = 'settings-row';
  const mobileEnableText = document.createElement('div');
  mobileEnableText.className = 'settings-row-text';
  const mobileEnableLabel = document.createElement('div');
  mobileEnableLabel.className = 'settings-row-label';
  mobileEnableLabel.textContent = 'Enable mobile check-in / Read to Earn';
  const mobileEnableHint = document.createElement('div');
  mobileEnableHint.className = 'settings-row-hint';
  mobileEnableHint.textContent = 'Uses the protected OAuth token and stops on a missing balance increase.';
  mobileEnableText.appendChild(mobileEnableLabel);
  mobileEnableText.appendChild(mobileEnableHint);
  const mobileToggleWrap = document.createElement('span');
  mobileToggleWrap.className = 'toggle-compact';
  const mobileToggle = document.createElement('input');
  mobileToggle.type = 'checkbox';
  mobileToggle.className = 'mobile-tasks-enabled';
  mobileToggle.checked = mobileCfg.enabled !== false;
  const mobilePill = document.createElement('span');
  mobilePill.className = 'toggle-pill';
  mobileToggleWrap.appendChild(mobileToggle);
  mobileToggleWrap.appendChild(mobilePill);
  mobileEnableRow.appendChild(mobileEnableText);
  mobileEnableRow.appendChild(mobileToggleWrap);
  mobileGroup.appendChild(mobileEnableRow);

  const mobileSubRow = document.createElement('div');
  mobileSubRow.className = 'form-grid-2';
  const checkField = make_form_field('Check-in', 'checkbox', 'mobile-check-in-enabled', '', {});
  const readField = make_form_field('Read to Earn', 'checkbox', 'mobile-read-enabled', '', {});
  checkField.querySelector('input').checked = mobileCfg.check_in_enabled !== false;
  readField.querySelector('input').checked = mobileCfg.read_to_earn_enabled !== false;
  mobileSubRow.appendChild(checkField);
  mobileSubRow.appendChild(readField);
  mobileGroup.appendChild(mobileSubRow);

  const mobileNumbers = document.createElement('div');
  mobileNumbers.className = 'form-grid-2';
  mobileNumbers.appendChild(make_form_field('Max articles', 'number', 'mobile-max-articles', mobileCfg.max_articles != null ? mobileCfg.max_articles : 10, { min: 1, max: 10 }));
  mobileNumbers.appendChild(make_form_field('Target points', 'number', 'mobile-target-points', mobileCfg.target_points != null ? mobileCfg.target_points : 30, { min: 1, max: 100 }));
  mobileGroup.appendChild(mobileNumbers);
  const mobileDelays = document.createElement('div');
  mobileDelays.className = 'form-grid-2';
  mobileDelays.appendChild(make_form_field('Read delay min (s)', 'number', 'mobile-delay-min', mobileCfg.read_delay_min != null ? mobileCfg.read_delay_min : 8, { min: 0, max: 3600 }));
  mobileDelays.appendChild(make_form_field('Read delay max (s)', 'number', 'mobile-delay-max', mobileCfg.read_delay_max != null ? mobileCfg.read_delay_max : 18, { min: 0, max: 3600 }));
  mobileGroup.appendChild(mobileDelays);

  const mobileStatus = document.createElement('div');
  mobileStatus.className = 'form-hint mobile-task-status';
  const currentStatus = item.mobile_status || {};
  const statusRead = currentStatus.read_to_earn || {};
  mobileStatus.textContent = currentStatus.result
    ? `Today: ${currentStatus.result} · Read ${statusRead.articles || 0}/${statusRead.target_articles || mobileCfg.max_articles || 10} · ${statusRead.points || 0}/${statusRead.target_points || mobileCfg.target_points || 30}`
    : 'Today: not attempted';
  mobileGroup.appendChild(mobileStatus);

  const mobileActions = document.createElement('div');
  mobileActions.className = 'run-actions';
  const authBtn = document.createElement('button');
  authBtn.type = 'button';
  authBtn.className = 'btn-secondary';
  authBtn.textContent = 'Connect / re-authorize';
  authBtn.addEventListener('click', () => {
    show_toast('Opening the visible Edge authorization window…', 'info', { duration: 6000 });
    pywebview.api.connect_mobile_tasks(item.id).then(result => {
      if (result && result.ok) show_toast('Mobile authorization saved.', 'success');
      else show_toast((result && result.error) || 'Authorization failed.', 'error');
    });
  });
  mobileActions.appendChild(authBtn);
  mobileGroup.appendChild(mobileActions);
  body.appendChild(mobileGroup);
  ...
  const refreshSummary = () => {
    ...
    liveSched.mobile_tasks = card.querySelector('.mobile-tasks-enabled').checked;
...
async function save_settings() {
  ...
  for (const card of cards) {
    ...
    const dashboardVariant = dashEl && ['auto', 'legacy', 'new'].includes(dashEl.value)
      ? dashEl.value : 'auto';
    const mobileTasksEnabled = card.querySelector('.mobile-tasks-enabled').checked;
    const checkInEnabled = card.querySelector('.mobile-check-in-enabled').checked;
    const readEnabled = card.querySelector('.mobile-read-enabled').checked;
    const maxArticles = parseInt(card.querySelector('.mobile-max-articles').value, 10);
    const targetPoints = parseInt(card.querySelector('.mobile-target-points').value, 10);
    const delayMin = parseInt(card.querySelector('.mobile-delay-min').value, 10);
    const delayMax = parseInt(card.querySelector('.mobile-delay-max').value, 10);
    ...
    if (enabled) {
      ...
      if ((pc || 0) + (mobile || 0) === 0 && !mobileTasksEnabled) {
    ...
    if (isNaN(maxArticles) || maxArticles < 1 || maxArticles > 10) {
      show_toast('Max articles must be between 1 and 10.', 'warning');
      return;
    }
    if (isNaN(targetPoints) || targetPoints < 1 || targetPoints > 100) {
      show_toast('Target points must be between 1 and 100.', 'warning');
      return;
    }
    if (isNaN(delayMin) || isNaN(delayMax) || delayMin < 0 || delayMax < delayMin || delayMax > 3600) {
      show_toast('Read delay must be a valid 0–3600 second range.', 'warning');
      return;
    }
    ...
    payloads.push({
      ...
      mobileTasks: {
        enabled: mobileTasksEnabled,
        check_in_enabled: checkInEnabled,
        read_to_earn_enabled: readEnabled,
        max_articles: maxArticles,
        target_points: targetPoints,
        read_delay_min: delayMin,
        read_delay_max: delayMax,
      },
    });
  }
  ...
  try {
    ...
    const mobileCalls = payloads.map(p =>
      pywebview.api.set_mobile_task_config(p.id, p.mobileTasks)
    );
    ...
    const results = await Promise.all([...scheduleCalls, ...mobileCalls, startupCall, closeToTrayCall]);
    const startupOk = results[results.length - 2];
    const scheduleResults = results.slice(0, cards.length);
    const mobileResults = results.slice(cards.length, cards.length * 2);
    const failures = [...scheduleResults, ...mobileResults].filter(ok => !ok).length;
...
function refresh_account_ui() {
  ...
  pywebview.api.list_accounts().then(accounts => {
    ...
    const mobileBtn = document.getElementById('mobile_tasks_btn');
    if (mobileBtn) mobileBtn.disabled = !shouldEnable;
