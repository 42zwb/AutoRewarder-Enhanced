...
def _run_once(api, pc, mobile, include_mobile_tasks=True, mobile_tasks_only=False):
    """
    ...
    try:
        return api.main(
            ...
            mobile_tasks_only=mobile_tasks_only,
        )
    except Exception as e:
...
def _run_scheduled(
    ...
    if total <= 0:
        if include_mobile_tasks:
            console_log("No searches scheduled; running mobile tasks once.")
            try:
                api.main(0, 0, include_mobile_tasks=True)
            except Exception as e:
                console_log(f"[ERROR] Mobile-only scheduled run failed: {e}")
        else:
...
def _run_account(
    ...
    Args:
        ...
        mobile_tasks_only: run only API-backed mobile tasks
        skip_mobile_tasks: do not run API-backed mobile tasks in a normal run
    ...
    if pc_override is None and mobile_override is None and not mobile_tasks_only:
        if not sched.get("enabled"):
            if not meta.get_mobile_tasks().get("enabled") or skip_mobile_tasks:
                console_log(f"Skipping '{label}': schedule disabled.")
                return False
        today = date.today().isoformat()
        if (
            not mobile_tasks_only
            and not force
            and sched.get("last_triggered_date") == today
        ):
            console_log(f"Skipping '{label}': already triggered today.")
            return False

    if mobile_tasks_only and not meta.get_mobile_tasks().get("enabled"):
        console_log(f"Skipping '{label}': mobile tasks disabled.")
        return False
    ...
    if mobile_tasks_only:
        _run_once(api, 0, 0, include_mobile_tasks=True, mobile_tasks_only=True)
    elif sched.get("advancedScheduling") and (
        ...
        _run_scheduled(
            ...
            include_mobile_tasks=not skip_mobile_tasks,
        )
    else:
        _run_once(api, pc, mobile, include_mobile_tasks=not skip_mobile_tasks)
...
def main():
    ...
    parser.add_argument(
        "--mobile-tasks-only",
        action="store_true",
        help="Run only mobile check-in and Read to Earn tasks.",
    )
    parser.add_argument(
        "--skip-mobile-tasks",
        action="store_true",
