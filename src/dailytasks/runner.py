...
class DailySet:
    ...
    def __init__(self, status_file, logger=None, dashboard_variant="auto"):
        ...
        self.last_sections = {
            "Daily Set": {"status": "unknown"},
            "More Activities": {"status": "unknown"},
        }
    ...
    def perform_daily_set(self, driver, human, stop_event=None, variant=None):
        ...
        if variant == "new":
            ...
            self.last_sections = {
                ...
                "More Activities": {
                    "status": handler.more_status,
                    "total": handler.more_total,
                    "final": handler.more_completed,
                },
            }
            return result
    ...
    def _perform_legacy(self, driver, human, stop_event=None):
        ...
        try:
            ...
            for pass_index in range(3):
                if pass_index > 0:
                    self._log(
                        f"More Activities/Daily Set verification pass {pass_index + 1}/3 — reloading dashboard."
                    )
                    try:
                        driver.get(DASHBOARD_URL)
                        WebDriverWait(driver, 15).until(
                ...
                for section_name, selector in SECTIONS:
                    ...
                    pass_sections[section_name] = dict(section_result)
                    for k in pass_totals:
                        pass_totals[k] += section_result[k]

                totals["newly"] += pass_totals["newly"]
                totals["attempted"] += pass_totals["attempted"]
                last_pass = pass_totals
                self.last_sections = {
                    name: {
                        **result,
                        "status": (
                            "completed"
                            if result["total"] == result["final"]
                            else "partial"
                        ),
                    }
                ...
                if pass_totals["attempted"] == 0:
                    break
                if pass_index < 2 and stop_event is not None and stop_event.is_set():
                    break

            if last_pass is not None:
                totals["already"] = last_pass["already"]
                totals["final"] = last_pass["final"]
                totals["total"] = last_pass["total"]
            ...
            self._log(
                "[WARNING] Rewards tasks remain incomplete after 3 verification passes. "
                "Leaving today's status partial so the next run can retry."
            )
            return False
