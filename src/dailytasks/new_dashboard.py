...
# Lightweight diagnostics for the cards intentionally filtered out of the
# actionable list above. This keeps logs honest when a region shows a promo,
# locked card, or a loaded section with no point badge.
_MORE_ACTIVITIES_DIAG_JS = r"""
try {
  var root = document.getElementById('moreactivities');
  if (!root) return { loaded: false, links: 0, completed: 0, locked: 0, no_points: 0 };
  var out = { loaded: true, links: 0, completed: 0, locked: 0, no_points: 0 };
  var links = root.querySelectorAll('a[href]');
  for (var i = 0; i < links.length; i++) {
    var a = links[i], href = a.href || '';
    if (href.indexOf('bing.com') < 0 || href.indexOf('/earn/quest/') >= 0) continue;
    out.links++;
    if (a.querySelector('[class*="statusSuccess"]')) { out.completed++; continue; }
    if (a.getAttribute('aria-disabled') === 'true' || a.getAttribute('data-disabled') === 'true') { out.locked++; continue; }
    var txt = (a.textContent || '').replace(/\s+/g, ' ').trim();
    if (!/\+\s*\d+/.test(txt)) out.no_points++;
  }
  return out;
...
class NewDashboardDailySet:
    ...
    def perform(self, driver, human, stop_event=None):
        ...
        # More Activities is deliberately independent from the Daily Set:
        # opening a link is not enough, and an unverified remainder keeps the
        # overall result partial instead of marking the day Done.
        return daily_ok and self.more_status in ("completed", "already_done")
    ...
    def _run_more_activities(self, driver, human, stop_event=None):
        """
        Click and verify the incomplete point-earning cards in /earn.

        Microsoft may acknowledge a navigation without crediting the activity.
        Therefore each pass re-loads the page and uses the current incomplete
        card set as the only completion proof. At most three passes are made;
        direct navigation is a last-resort attempt and is never counted as
        completed until the next page read confirms the card disappeared.
        """
        ...
        for pass_index in range(3):
            if stop_event is not None and stop_event.is_set():
                self.more_status = "stopped"
                break
            try:
                driver.get(EARN_URL)
                if not self._wait_for(driver, "#moreactivities", timeout=15):
                    self._log(
            ...
            try:
                ...
                if isinstance(diagnostics, dict):
                    self._log(
                        "'earn-page' diagnostics: "
                        f"links={diagnostics.get('links', 0)}, "
                        f"completed={diagnostics.get('completed', 0)}, "
                        f"locked={diagnostics.get('locked', 0)}, "
                        f"no_points/promo={diagnostics.get('no_points', 0)}."
                    )
            except Exception:
                pass
            if pass_index == 0:
                initial = dict(current)
                self.more_total = len(initial)
                if not initial:
            ...
            self._log(
                f"'earn-page' pass {pass_index + 1}/3: {len(current)} incomplete activity(ies)."
            )
            main_tab = driver.current_window_handle
            for dest, item in current.items():
                if stop_event is not None and stop_event.is_set():
                    self.more_status = "stopped"
                    break
                ...
                clicked = anchor is not None and self._click_anchor(
                    driver,
                    human,
                    anchor,
                    main_tab,
            ...
            # The next iteration reloads /earn and proves whether these
            # attempts actually changed the incomplete-card set.
            if pass_index < 2 and self.more_status == "stopped":
                break

            remaining = set(current)
            remaining_initial = set(initial).intersection(remaining)
            completed = max(0, len(initial) - len(remaining_initial))
            self.more_completed = min(self.more_total, completed)
            if not remaining:
                self.more_status = "completed"
                break

        if self.more_status not in ("completed", "stopped"):
            self.more_status = "partial" if initial else "unavailable"
        verified = max(0, min(self.more_total, self.more_completed))
        self.last_totals["earn"] = self.last_totals.get("earn", 0) + verified
        self.last_totals["attempted"] = self.last_totals.get("attempted", 0) + attempted
        self._log(
            f"'earn-page': verified {verified}/{self.more_total}; attempts={attempted}; "
            f"status={self.more_status}."
        )
