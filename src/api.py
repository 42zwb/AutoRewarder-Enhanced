...
class AutoRewarderAPI:
    ...
    def _rebuild_account_context(self):
        ...
        if current_id:
            ...
            self.mobile_task_runner = MobileTaskRunner(
                current_id,
                mobile_status_path(current_id),
                mobile_token_path(current_id),
                driver_manager=self.driver_manager,
                config=self.account_meta.get_mobile_tasks(),
                logger=self.log,
    ...
    def get_mobile_task_config(self, account_id=None):
        """Return validated mobile task settings for one account."""
        account_id = account_id or self.account_manager.current_id()
        if not account_id or not self.account_manager.exists(account_id):
            return None
        return AccountMetaManager(account_id).get_mobile_tasks()

    def get_mobile_task_status(self, account_id=None):
        """Return today's mobile task status without exposing any token data."""
        account_id = account_id or self.account_manager.current_id()
        if not account_id or not self.account_manager.exists(account_id):
            return None
        runner = MobileTaskRunner(
            account_id,
            mobile_status_path(account_id),
            mobile_token_path(account_id),
            config=AccountMetaManager(account_id).get_mobile_tasks(),
            logger=self.log,
        )
        return runner.get_status()

    def set_mobile_task_config(self, account_id, payload):
        """Persist mobile task settings and update the active runner."""
        if not account_id or not self.account_manager.exists(account_id):
            return False
        ...
        if ok:
            self.log(f"Mobile task settings saved for account {account_id}.")
        return ok

    def connect_mobile_tasks(self, account_id=None):
        """Run the visible one-time OAuth authorization flow for an account."""
        account_id = account_id or self.account_manager.current_id()
        if not account_id or not self.account_manager.exists(account_id):
        ...
        if account_id != self.account_manager.current_id():
            # Switching accounts starts an asynchronous balance refresh. Do
            # not race that driver against the interactive OAuth profile.
            return {"ok": False, "error": "select_account_first"}
        try:
            ...
            self.log(f"[WARNING] Mobile OAuth authorization failed: {exc}")
            return {"ok": False, "error": str(exc)[:240]}
        except Exception as exc:
            self.log(f"[WARNING] Mobile OAuth authorization failed: {exc}")
            return {"ok": False, "error": "authorization_failed"}

    def get_all_schedules(self):
        """Return schedule + mobile settings/status for the settings modal."""
        result = []
            ...
            result.append(
                {
                    ...
                    "mobile_tasks": meta.get_mobile_tasks(),
                    "mobile_status": self.get_mobile_task_status(acc["id"]),
    ...
    def set_schedule(self, account_id, payload):
        ...
        `payload` accepts the schedule fields enabled, advancedScheduling,
        runDuration (1..24), queriesPerHour (1..99), queries_pc (0..130),
        queries_mobile (0..99), run_time (HH:MM 24h). Unknown keys are ignored.
...
- Close the browser when you're done.""")
            ...
            result.append(
                {
                    ...
                    "check_ins": stats["lifetime"].get("check_ins", 0),
                    "read_articles": stats["lifetime"].get("read_articles", 0),
                    "read_points": stats["lifetime"].get("read_points", 0),
                    "mobile_task_runs": stats["lifetime"].get("mobile_task_runs", 0),
                }
            )
        return result
    ...
    def _run_mobile_tasks(self, force=False):
        """Run the API-backed mobile activities once and fold their deltas into this session."""
        runner = self.mobile_task_runner
        if runner is None:
            self.log("[WARNING] Mobile tasks unavailable: no account context.")
            return None
        ...
        before_check_ok = before_check.get("status") in ("completed", "already_done")
        after_check_ok = after_check.get("status") in ("completed", "already_done")
        if after_check_ok and not before_check_ok:
            self._session_counts["check_ins"] += 1
        self._session_counts["read_articles"] += max(
            0, _number(after_read, "articles") - _number(before_read, "articles")
        )
        self._session_counts["read_points"] += max(
            0, _number(after_read, "points") - _number(before_read, "points")
        )
        self._session_counts["mobile_task_runs"] += 1
        self.log(
            "Mobile tasks: "
            f"status={summary.status}, check_in={summary.check_in_status}, "
            f"read={summary.read_articles}/{summary.target_articles}, "
            f"read_points={summary.read_points}/{summary.target_points}"
    ...
    def main(
        ...
        mobile_tasks_only=False,
    ):
        """
        Run the bot against the currently-selected account.

        Default mode (daily_only=False): runs sequentially
          1. Mobile check-in / Read to Earn API tasks (once per top-level run).
          2. PC phase     — desktop UA, `pc_count` Bing searches, then Daily Set.
          3. Mobile phase — iPhone UA, `mobile_count` Bing searches only.
        Either count may be 0 to skip that phase.

        Daily-only mode (daily_only=True): runs mobile API tasks first, then
        skips searches and opens a desktop driver for Daily Set + More Activities.
        ``mobile_tasks_only`` skips the browser Daily Set as well.
        ...
        Args:
            ...
            include_mobile_tasks (bool): whether to run the mobile API tasks
            mobile_tasks_only (bool): whether to run only the mobile API tasks
        """
        if self.account_manager.current_id() is None:
        ...
        include_mobile_tasks = bool(include_mobile_tasks)
        ...
        if self.account_meta is not None:
            try:
                mobile_config_enabled = bool(
                    self.account_meta.get_mobile_tasks().get("enabled")
                )
            except Exception:
                mobile_config_enabled = False
        ...
        if (
            ...
            and pc_count == 0
            and mobile_count == 0
            and not (include_mobile_tasks and mobile_config_enabled)
        ):
            self.log("[WARNING] Nothing to do (PC and Mobile counts are both 0).")
            if self._webview_window:
                self._webview_window.evaluate_js("enable_start_button()")
        ...
        schedule = {}
        if not daily_only and not mobile_tasks_only and self.account_meta is not None:
            try:
        ...
        try:
            ...
            if mobile_tasks_only:
                self.log("Starting mobile tasks only...")
            elif daily_only:
    ...
    def _record_session_stats(self):
        ...
        try:
            self.stats.record_session(
                ...
                check_ins=self._session_counts.get("check_ins", 0),
                read_articles=self._session_counts.get("read_articles", 0),
                read_points=self._session_counts.get("read_points", 0),
                mobile_task_runs=self._session_counts.get("mobile_task_runs", 0),
