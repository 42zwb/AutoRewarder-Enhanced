...
#         meta.json          (per-account: first_setup_done, schedules, mobile task settings)
#         mobile_status.json (per-account: today's mobile task state)
...
def mobile_status_path(account_id):
    """Return the per-account mobile task status path."""
    return os.path.join(account_dir(account_id), "mobile_status.json")


def mobile_token_path(account_id):
    """Return the DPAPI-protected mobile OAuth token path."""
    return os.path.join(account_dir(account_id), "mobile_token.bin")
