...
### Known issue: Bing search credit can fail intermittently

On 2026-09-13, both the desktop and mobile Selenium search paths were reproduced completing their browser actions while Microsoft Rewards awarded no search points. The account remained signed in and an immediately following manual search earned 3 points, while the authenticated Rewards counter stayed unchanged after each automated test. On the next day, 2026-09-14, the same machine and account were observed completing the daily workflow and receiving search credit normally, without a change to the search implementation. This strongly suggests an intermittent Microsoft-side eligibility, risk, or temporary-limiting state rather than a permanent failure of the local search code; it does not guarantee that a later run will be credited.
...
There is no supported automatic workaround for a server rejection of automated Rewards searches. Changing fingerprints, hiding WebDriver, or adding “human-like” timing would attempt to evade the eligibility controls and is intentionally not implemented here. If a run returns `manual_required`, record the date and counter state, avoid repeated rapid retries, and verify again later; use manually entered, genuine searches whenever possible. If manual searches also stop receiving credit, wait for any temporary earning limitation to clear or contact Microsoft Rewards Support. The application’s `manual_required` result is a truthful hand-off, not proof of a permanent browser or account failure.
...
### 已知问题：Bing 搜索积分可能间歇性失效

2026-09-13 曾复现桌面端和移动端 Selenium 搜索均能完成浏览器动作，但 Microsoft Rewards 不增加搜索积分。当时账号保持登录，紧接着进行的一次人工搜索可以获得 3 分，而每次自动化测试后的官方 Rewards 计数均不变。2026-09-14，在没有修改搜索实现的情况下，同一台电脑和同一账号的每日任务及搜索又恢复了正常计分。这更支持“微软服务端资格、风控或临时限制状态会间歇性变化”的判断，而不是本地搜索代码永久失效；但这也不代表下一次运行一定会计分。

微软把符合条件的 Rewards 搜索定义为用户为了真实个人研究而手动输入的搜索，并明确排除机器人、宏和其他自动化方式；官方支持页也明确要求不要使用程序辅助搜索（[微软服务协议](https://www.microsoft.com/en-us/servicesagreement)、[Rewards 搜索限制说明](https://support.microsoft.com/en-au/accounts-billing/rewards/limiting-your-searches-in-microsoft-rewards)）。

代码现已改为以登录账号返回的 Rewards `pcSearch` 计数为准，而不是把 Selenium 按下回车当作成功；同时验证 Bing 结果页 URL、移除不稳定的图片/视频/新闻随机标签跳转，并在剩余批次前只提交一次探测搜索。若服务端计数不增长，则计分搜索记为 0、返回退出码 `3`（`manual_required`）并立即停止；其他部分失败返回退出码 `2`。程序不再误报 `Done!` 或持续重复提交。该修改解决的是“程序误报成功”问题，**不会**绕过微软的资格判定，也不能保证自动搜索获得积分。由于问题可能在下一天自行恢复，`manual_required` 应被记录为带日期的诊断结果，而不是直接认定账号或代码永久失效。本次复现没有覆盖移动签到、阅读以赚取、Daily Set 和 More Activities，因此本说明不对这些任务的状态作判断。

受影响日期的账号实测已经排除常见的本机原因：账号处于登录状态，Rewards 用户信息接口可以正常返回余额和搜索进度 `18/60`，桌面端能进入正确的 Bing 结果页；移动会话也确实报告 iPhone UA、`412×915` 视口、5 个触点和粗指针。桌面、移动两条自动路径都没有改变服务端计数，而同一账号随后进行一次人工搜索，进度立即从 `15/60` 变为 `18/60`，余额增加 3 分。用户信息响应中也没有公开的冷却或限制标志。因此，受影响日期的剩余判定发生在微软服务端的搜索资格层，而不是界面缓存、登录状态、查询次数或 mobile 模拟开关；下一天恢复正常则说明该服务端状态并非固定不变。
...
服务端暂时拒绝自动搜索后，不存在受支持的自动修复办法。更换指纹、隐藏 WebDriver 或进一步增加“拟人化”节奏都属于尝试规避资格控制，本仓库不会实现。此时应记录当天的状态、避免快速重复提交，并改为用户亲自输入、用于真实查询的搜索；如果人工搜索也不再计分，可在稍后或下一天再次验证，必要时联系 Microsoft Rewards 支持。程序返回的 `manual_required` 是准确的人工接管状态，不是一个可以靠重启浏览器必然恢复的错误，也不是永久失效的证明。
