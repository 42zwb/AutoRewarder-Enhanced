...
- Before browser work, it runs mobile check-in and Read to Earn once. More Activities is reloaded and verified for up to three passes; an opened page is not treated as completion until the card state or points change confirms it
...
### Mobile check-in and Read to Earn

The mobile-task module runs before the browser phases in a normal session and only once in an advanced scheduled run. It performs two independent activities through the Microsoft Rewards mobile activity API:

- **Daily check-in:** the response must show a positive balance delta or an explicit already-completed state.
- **Read to Earn:** the current region's offer is discovered from the account profile, then at most 10 unique submissions are paced by the configured delay. The default acceptance target is the actual `30/30` points, not merely opening ten URLs.

#### First-time authorization

1. Open **Settings** and expand the account's schedule card.
2. In **Mobile tasks**, click **Connect / re-authorize**.
3. Complete the Microsoft authorization in the visible Edge window using that account's existing browser profile; do not provide a password to AutoRewarder.
4. Save the mobile-task toggles and run **Mobile tasks only** from the main window for the first diagnostic run.

The refresh token is encrypted with Windows DPAPI and tied to the current Windows user and account directory. Access tokens are memory-only and are not written to logs. If authorization is absent or rejected, the run is recorded as `auth_required` and the old search/Daily Set pipeline can continue.

The status file is `%LOCALAPPDATA%\AutoRewarder\accounts\<account_id>\mobile_status.json`. Its result is one of `completed`, `already_done`, `partial`, `unavailable`, `auth_required`, `failed`, `stopped`; only the first two count as success. A `partial` result is intentional when an article produces no new points, because repeating the same activity can create useless requests or risk the account.

The API route is region-dependent and may change. If the region does not expose a read offer, or the final Rewards page does not reach `30/30`, inspect the status file and leave the task for manual handling. Microsoft Rewards eligibility and automation rules vary by region; see the [official Microsoft Rewards guidance](https://support.microsoft.com/en-us/accounts-billing/rewards/learn-about-microsoft-rewards-points).

### Mobile tasks only and CLI fallback

The main window's **Mobile tasks only** button runs check-in/Read to Earn without opening the search or Daily Set browser flow. Equivalent headless commands are:

```text
AutoRewarder.exe --headless --account Main --mobile-tasks-only
AutoRewarder.exe --headless --account Main --skip-mobile-tasks
```

`--skip-mobile-tasks` is a diagnostic fallback for keeping the pre-existing browser automation unchanged. With advanced scheduling, the mobile API pass is sent only in the first batch; later batches run searches only.
...
- **API key:** Paste a key from your chosen provider. On Windows it is stored separately with DPAPI; on non-Windows development environments it is kept in a separate mode-600 file. It is only sent to that provider to request queries — no personal data is shared. Older `settings.json` files are migrated when possible; remove/re-enter the key if migration cannot access the protected store.
...
```
%LOCALAPPDATA%\AutoRewarder\accounts\<account_id>\history.json
```
...
```
%LOCALAPPDATA%\AutoRewarder\background_log.txt
```
...
**Mobile task result is `auth_required`:**
- Open Settings → the account card → **Connect / re-authorize** and finish the visible Microsoft authorization
- Confirm that the Edge profile is signed in to the intended Rewards account
- Retry with **Mobile tasks only**; no password is collected or stored by AutoRewarder

**Mobile task result is `unavailable` or `partial`:**
- Check `mobile_status.json` and the background log for the region, offer and last error
- Confirm the Rewards page itself offers check-in/Read to Earn and that the balance changed
- Do not repeatedly force the same activity when the response reports no positive balance delta; use the manual Rewards page as the fallback
